...
class Image_GD_Driver extends Image_Driver {
  ...
  public function addText($properties)
  {
    $img = $this->tmp_image;

    // 获得文字各定点起始位置
    $corners = imagettfbbox($properties['size'],$properties['angle'],$properties['fontfile'],$properties['text']);
    
    imagettftext(
      $img,
      $properties['size'],
      $properties['angle'],
      $properties['x']-abs($corners[2])-5, // 文字左下顶点横坐标
      $properties['y'], // 文字左下顶点纵坐标
      imagecolorallocate($img,$properties['color'][0],$properties['color'][1],$properties['color'][2]),
      $properties['fontfile'],
      $properties['text']
    );
