...
Yii::import('application.components.image.Image_Driver');
...
class Image {
   ...
   * Add text
   */
  public function addText($text, $size, $x, $y, $color, $fontfile=null, $angle=0)
  {
    if(!$fontfile)
      $fontfile = Yii::getPathOfAlias('webroot').'/fonts/arial.ttf';
    
    $this->actions['addText'] = array(
      'text' => $text,
      'size' => $size,
      'angle' => $angle,
      'x' => $x,
		...
		if ($status = $this->driver->process($this->image, $this->actions, $dir, $file))
		{
			if ($chmod !== FALSE)
			{
				...
				if (!is_file($new_image)) {
				    exit("image:{$new_image} is not found.");
				}
